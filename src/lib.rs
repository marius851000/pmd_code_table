...
impl CodeTable {
    ...
    pub fn add_missing(&mut self) {
        self.entries.push(CodeTableEntryFile {
            string: "rubi:base".to_string(),
            value: 0xcf00,
            flags: 0,
            lenght: 0,
            unk: 0,
    ...
    /// Generate a [`TextToCode`] object, allowing to transform a human-readable string (generated by a [`CodeToText`]) into a
    /// binary text readable by the game (it first need to be put inside a message container (see the ``pmd_message`` crate) then
    /// into a farc file (see ``pmd_farc`` crate). The ``pmdtranslate`` crate contain command line and a complete exemple for those case.
